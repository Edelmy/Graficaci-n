...
void setup() {
  ...
  // Dibuja histograma
  int alto = 100;        // altura máxima de las barras
  int baseY = 512 + 10;      // empieza debajo de la imagen
  stroke(255);                  
  strokeWeight(2);              

  float barraAncho = (float)512 / 256; // ancho de cada barra
  for (int i = 0; i < 256; i++) {
    float altura = map(histograma[i], 0, mayor, 0, alto);
    float x = i * barraAncho;
    line(x, baseY + alto, x, baseY + alto - altura);
  }
